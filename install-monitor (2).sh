...
URL_BINARIO="https://api.github.com/repos/bmanzano/resource_tmp/contents/validator_arm"
URL_YAML="https://api.github.com/repos/bmanzano/resource_tmp/contents/config.yaml"
URL_YAML_SIGNAL="https://api.github.com/repos/bmanzano/resource_tmp/contents/config-signal.yaml"
...
update_service() {
    ...
    if download_file "$URL_YAML" "$CONFIG_PATH.new" "nueva configuracion"; then
        ...
        log_success "Configuracion actualizada"
    else
        rm -f "$CONFIG_PATH.new"
        log_warning "No se pudo actualizar la configuracion"
    fi
